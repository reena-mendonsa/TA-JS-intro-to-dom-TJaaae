...
// Select all the hr elements and store it in a variable named 
//allHrs using querySelectorAll
let allHrs = document.querySelectorAll('hr');
/* 
Convert the NodeList returned by querySelectorAll to Array using Array.from() or
 spread operator and store it in allHrsArray

Array.from converts an array kind of data into array so we can use methods like map,
 reduce
...
let arrayHrImg = Array.from(hrimg);
for(borderImg of arrayHrImg){
    
  borderImg.style.border='2px solid purple';   
...
// Change the inner text of para to "querySelector returns an element you 
//can maupulate but querySelectorAll returns the collection of elements in
...
// Select box 2 and append a new paragraph element with content
// "Append inserts as last child" just after hr element.
let box2 = document.querySelector('.two');
let para2 = document.createElement('p');
para2.innerText='Append inserts as last child';
box2.append(para2);

// Select box 3 and prepend a new paragraph element with content 
//"Prepend inserts as first child" just before hr element.
...
for(let i=0 ; i < allBoxs.length ; i=i+2){
    
  allBoxs[i].style.backgroundColor =' aliceblue';   
 }
...
box9.append(btn);
// Create a img element with src value `https://images.unsplash.com/photo-1592500595497-d1f52a40b207?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=400&q=80` 
...
a1.append(document.createTextNode = 'AltCampus');
a2.append(document.createTextNode = 'Google');
